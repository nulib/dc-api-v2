...
class TestCheckpointSaver(unittest.TestCase):
    ...
    def test_checkpoint_saver_initialization(self, mock_checkpointer):
        ...
        mock_checkpointer.assert_called_once_with(
            bucket_name="test-bucket", retain_history=False, **kwargs
        )
...
class TestOpenSearchEndpoint(unittest.TestCase):
    def test_opensearch_endpoint_with_full_url(self):
        with patch.dict(
            os.environ, {"OPENSEARCH_ENDPOINT": "https://test.amazonaws.com"}
        ):
...
class TestOpenSearchClient(unittest.TestCase):
    ...
    def test_opensearch_client_initialization(
        self, mock_opensearch, mock_aws4auth, mock_session
    ):
        ...
        with patch.dict(
            os.environ,
            {"AWS_REGION": "us-west-2", "OPENSEARCH_ENDPOINT": "test.amazonaws.com"},
        ):
...
class TestOpenSearchVectorStore(unittest.TestCase):
    ...
    def test_opensearch_vector_store_initialization(
        self, mock_neural_search, mock_aws4auth, mock_session
    ):
        # Setup mock credentials
        mock_credentials = MagicMock()
        mock_session.return_value.get_credentials.return_value = mock_credentials
...
class TestWebsocketClient(unittest.TestCase):
    ...
    def test_websocket_client_with_env_endpoint(self, mock_boto3_client):
        with patch.dict(
            os.environ, {"APIGATEWAY_URL": "https://test-ws-env.amazonaws.com"}
        ):
