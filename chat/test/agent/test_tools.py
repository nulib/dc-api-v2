...
class TestTools(TestCase):
    ...
    def test_discover_fields(self, mock_opensearch):
        ...
        mock_client.indices.get_mapping.return_value = {
            "index_name": {
                "mappings": {
                    "properties": {
                        "field1": {"type": "keyword"},
                        "field3": {"properties": {"subfield1": {"type": "keyword"}}},
                    }
                }
            }
        }
        ...
        response = discover_fields.invoke(
            {"query": ""}
        )  # Assuming query is the required parameter
    ...
    def test_search(self, mock_opensearch):
        ...
        mock_results = [
            MockDoc(content=doc["id"], metadata=doc) for doc in expected_results
        ]
    ...
    def test_aggregate(self, mock_opensearch):
        mock_response = json.dumps({"aggregations": {"example_agg": {"buckets": []}}})
        ...
        response = aggregate.invoke(
            {"agg_field": "field1", "term_field": "term_field", "term": "term"}
        )
    ...
    def test_aggregate_no_term(self, mock_opensearch):
        mock_response = json.dumps({"aggregations": {"all_docs": {"buckets": []}}})
        mock_opensearch.return_value.aggregations_search.return_value = mock_response

        response = aggregate.invoke(
            {"agg_field": "field1", "term_field": "", "term": ""}
        )
        self.assertIsInstance(response, str)
        self.assertEqual(json.loads(response), json.loads(mock_response))
    ...
    def test_get_keyword_fields(self):
        properties = {
            "field1": {"type": "keyword"},
            "field2": {"type": "text"},
            "field3": {"fields": {"raw": {"type": "keyword"}}},
        }
    ...
    def test_complex_mapping_get_keyword_fields(self):
        properties = {
            "field1": {
                "properties": {
                    "nested1": {"type": "keyword"},
                    "nested2": {"properties": {"subnested1": {"type": "keyword"}}},
                }
            },
            "field2": {"fields": {"raw": {"type": "keyword"}}},
        }
        result = get_keyword_fields(properties)
        self.assertEqual(
            set(result), {"field1.nested1", "field1.nested2.subnested1", "field2.raw"}
        )
    ...
    def test_meadow_mapping_get_keyword_fields(self):
        ...
        expected_result = [
            ...
            "date_created_edtf",
            "embedding_model",
            "file_sets.accession_number",
            "file_sets.download_url",
            "file_sets.id",
            "file_sets.label",
            "file_sets.mime_type",
            "file_sets.original_filename",
            "file_sets.representative_image_url",
            "file_sets.role",
            "file_sets.streaming_url",
            "file_sets.webvtt",
            "folder_name",
            "folder_number",
            "genre.facet",
