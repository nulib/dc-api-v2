...
class TestSocketCallbackHandler(TestCase):
    ...
    def test_on_llm_end_with_content(self):
        ...
        with patch.object(
            self.handler, "on_llm_end", wraps=self.handler.on_llm_end
        ) as mock:
    ...
    def test_on_tool_end_search(self):
        ...
        self.assertEqual(
            self.handler.artifacts,
            [
                {
                    "type": "source_urls",
                    "artifact": [
                        "https://example.edu/item/1",
                        "https://example.edu/item/2",
                    ],
                }
            ],
        )
    ...
    def test_on_tool_end_aggregate(self):
        ...
        self.handler.on_tool_end(output)
        self.assertEqual(
            self.handler.artifacts, [{"type": "aggregation", "artifact": {"count": 10}}]
        )
